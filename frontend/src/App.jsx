...
import UserProfileMenu from "./auth/UserProfileMenu.jsx";
import LoginModal from "./auth/LoginModal.jsx";
import AuditTrailModal from "./auth/AuditTrailModal.jsx";
import UserManagementModal from "./auth/UserManagementModal.jsx";
...
export default function App() {
  ...
  const [form, setForm] = useState({
    ...
    investigator: currentUser?.name || "Forensic Examiner",
  ...
  const loadCases = async () => {
    try {
      ...
      setCases(r || []);
      if (!active && r && r.length > 0) {
        // Pick first authorized case for this user
        const authorized = r.find((c) => hasCaseAccess(c.case_number)) || r[0];
        if (authorized) setActive(authorized.id);
      }
    } catch (e) {
  ...
  const loadCase = async (id) => {
    ...
    try {
      ...
      if (d?.case_number) {
        addAuditLog("CASE_ACCESSED", "CASE", d.case_number, `Loaded investigation workspace for ${d.title}`);
      }
    } catch (err) {
  ...
  const handleSelectEvent = (event) => {
    ...
    addAuditLog("EVENT_SELECTED", "ARTIFACT", `#${event.id}`, `${event.event_type} (${event.source_type})`);
  };
  ...
  const upload = async (file) => {
    if (!can("canImport")) {
      alert("Permission denied: Examiner or Admin role required to import evidence packages.");
      return;
    }
    ...
    try {
      ...
      setIngestModal(res);
      addAuditLog("EVIDENCE_IMPORTED", "PACKAGE", res.filename, `Imported ZIP container (SHA-256: ${res.sha256?.slice(0, 16)}...)`);
      await loadCases();
  ...
  const acquireEvidence = async () => {
    if (!can("canAcquire")) {
      alert("Permission denied: Examiner or Admin role required to trigger endpoint evidence acquisition.");
      return;
    }
    setBusy(true);
    try {
      const res = await api(`/api/cases/${active}/acquire`, {
        ...
        body: JSON.stringify({
          mode: acquireMode,
          policy: policy,
          notes: `Authorized acquisition by ${currentUser?.name || detail?.investigator || "Examiner"}`,
        }),
      }).then((x) => x.json());
      ...
      addAuditLog("EVIDENCE_ACQUIRED", "POLICY_AGENT", res.package_filename, `Acquisition mode: ${acquireMode}`);
      await loadCases();
      await loadCase(active);
    } catch (err) {
  ...
  const ask = async (customQ) => {
    ...
    setBusy(true);
    addAuditLog("AI_QUERY_INITIATED", "AI_COPILOT", "llama3.2:3b", questionText);
    try {
  ...
  const createCase = async () => {
    if (!can("canCreateCase")) {
      alert("Permission denied: Examiner or Admin role required to create investigations.");
      return;
    }
    await api("/api/cases", {
      method: "POST",
    ...
    addAuditLog("CASE_CREATED", "CASE", form.case_number, `Created investigation: ${form.title}`);
    setOpen(false);
    await loadCases();
  };

  const updateRec = async (id, status) => {
    if (!can("canVerifyEvidence")) {
      alert("Permission denied: Examiner role required to verify investigative tasks.");
      return;
    }
    ...
    addAuditLog("TASK_STATUS_UPDATED", "CHECKLIST", `#${id}`, `Status updated to ${status}`);
    await loadCase(active);
  };
  ...
  const verifyEvidence = async (evId) => {
    ...
    try {
      ...
      addAuditLog("EVIDENCE_HASH_VERIFIED", "EVIDENCE", `#${evId}`, res.ok ? "SHA-256 integrity check passed" : "Hash mismatch detected");
      await loadCase(active);
  ...
  // Case isolation filtering by RBAC
  const userAccessibleCases = useMemo(() => {
    return cases.filter((c) => hasCaseAccess(c.case_number));
  }, [cases, currentUser]);

  const filteredCases = useMemo(() => {
    if (!caseSearch.trim()) return userAccessibleCases;
    const cs = caseSearch.toLowerCase();
    return userAccessibleCases.filter(
      (c) =>
        c &&
  ...
  }, [userAccessibleCases, caseSearch]);

  const risk = inv?.risk_score || finding?.risk_score || 0;
  const isCaseAuthorized = detail ? hasCaseAccess(detail.case_number) : true;
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100vh", bgcolor: "#020806", color: "#eefaf4", overflow: "hidden", position: "relative" }}>
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            {active && (
              <Button
                ...
                target="_blank"
                onClick={() => addAuditLog("REPORT_EXPORTED", "REPORT", detail?.case_number, "Exported investigation report PDF")}
      ...
      <Box sx={{ flex: 1, display: "flex", position: "relative", overflow: "hidden", minHeight: 0 }}>
        ...
        {/* UNIFIED FORENSIC WORKSPACE GRID */}
        <Box
          component="main"
          sx={{
            flex: 1,
          ...
          >
            ...
            <Box sx={{ flexShrink: 0, bgcolor: "#050f0b", borderBottom: "1px solid rgba(61, 255, 174, 0.1)" }}>
              ...
              >
                ...
                <Tab icon={<HistoryIcon sx={{ fontSize: 15 }} />} iconPosition="start" label="Audit Trail" />
                {isAdmin && <Tab icon={<AdminPanelSettingsIcon sx={{ fontSize: 15 }} />} iconPosition="start" label="Users & Access" />}
              </Tabs>
            </Box>
            ...
            {!isCaseAuthorized ? (
              <Box sx={{ p: 4, textAlign: "center", my: "auto" }}>
                <LockIcon sx={{ color: "#ff6565", fontSize: 44, mb: 1.5 }} />
                <Typography variant="h6" sx={{ color: "#ff6565", fontWeight: 800 }}>
                  Case Access Restricted
                </Typography>
                <Typography variant="body2" sx={{ color: "#8fa89d", maxWidth: 440, mx: "auto", mt: 1 }}>
                  Your persona (<b>{currentUser.name}</b> · <code>{currentUser.role}</code>) does not have authorization to examine investigation <b>{detail?.case_number}</b>.
                </Typography>
                <Button
                  variant="outlined"
                  onClick={() => {
                    const firstAuth = cases.find((c) => hasCaseAccess(c.case_number));
                    if (firstAuth) setActive(firstAuth.id);
                  }}
                  sx={{ mt: 2, borderColor: "#3dffae", color: "#3dffae" }}
                >
                  Switch to Authorized Case
                </Button>
              </Box>
            ) : (
              <>
                ...
                {/* TAB 6: FORENSIC SECURITY AUDIT TRAIL */}
                {tab === 6 && (
                  <Box sx={{ p: 2.5, flex: 1, overflowY: "auto" }}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
                      <Box>
                        <Typography variant="subtitle1" sx={{ fontWeight: 800, color: "#3dffae" }}>
                          Security Audit Trail & Session Log
                        </Typography>
                        <Typography variant="caption" sx={{ color: "#8fa89d", fontSize: 11 }}>
                          Immutable chain-of-custody and user action records for this forensic session
                        </Typography>
                      </Box>
                      ...
                      <Button
                        ...
                        sx={{ borderColor: "rgba(61, 255, 174, 0.3)", color: "#3dffae", fontSize: 11 }}
                      >
                        Open Full Audit Console
                      </Button>
                    </Stack>

                    <Paper sx={{ p: 2, bgcolor: "#050f0b", border: "1px solid rgba(61, 255, 174, 0.12)", borderRadius: "10px" }}>
                      <Typography variant="body2" sx={{ color: "#8fa89d", mb: 2 }}>
                        Every evidence access, timeline query, AI synthesis, and case modification is cryptographically associated with the active persona (<b>{currentUser.name}</b> · <code>{currentUser.role}</code>) and logged to the local audit vault.
                      </Typography>
                    </Paper>
                  </Box>
                )}

                {/* TAB 7: USER MANAGEMENT CONSOLE (ADMIN ONLY) */}
                {tab === 7 && isAdmin && (
                  <Box sx={{ p: 2.5, flex: 1, overflowY: "auto" }}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
                      <Box>
                        <Typography variant="subtitle1" sx={{ fontWeight: 800, color: "#3dffae" }}>
                          Personnel Access & Case Scope Governance
                        </Typography>
                        <Typography variant="caption" sx={{ color: "#8fa89d", fontSize: 11 }}>
                          Manage forensic roles, activate accounts, and assign investigation permissions
                        </Typography>
                      </Box>
                      ...
                      <Button
                        ...
                        sx={{ bgcolor: "#3dffae", color: "#020806", fontWeight: 800, fontSize: 11 }}
                      >
      ...
      {/* Login / Authenticate Credentials Modal */}
      <LoginModal
        open={loginModalOpen}
      ...
      {/* Full User & Role Management Console Modal (Admin) */}
      <UserManagementModal
